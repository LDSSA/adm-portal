...
def u_has_profile(u: Any) -> bool:
    return u_accepted_coc(u) and getattr(u, "profile", None) is not None
...
def requires_candidate_profile(f: Callable[..., Any]) -> Callable[..., Any]:
    return user_passes_test(
        lambda u: u_has_profile(u) and not u.is_staff, redirect_field_name=None, login_url="/candidate/home"
    )(f)
